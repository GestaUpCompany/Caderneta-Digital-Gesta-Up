...
exports.extractLinkByPrefix = extractLinkByPrefix;
exports.extractLinkByPosition = extractLinkByPosition;
...
exports.getSubtiposDaFazenda = getSubtiposDaFazenda;
exports.findRowByInsumo = findRowByInsumo;
exports.calcularPrevisaoEstoque = calcularPrevisaoEstoque;
...
function getAuth() {
    ...
    const credentialsPath = path_1.default.resolve(CREDENTIALS_PATH);
 ...
 */
function extractLinkByPrefix(cellValue, prefix) {
    if (!cellValue)
        return null;
 ...
 * @returns Link extraído ou null se não encontrado
 */
function extractLinkByPosition(columnCValues, position) {
    if (!columnCValues || columnCValues.length === 0)
        return null;
    // Posição 1 = C2 (índice 0), Posição 2 = C3 (índice 1), Posição 3 = C4 (índice 2)
    const rowIndex = position - 1;
    if (rowIndex >= columnCValues.length)
        return null;
...
async function appendRow(spreadsheetUrl, sheetName, values) {
    const auth = getAuth();
    ...
    logger_1.logger.info(`Linha adicionada na aba ${sheetName}, linha ${rowNumber}`);
    return rowNumber;
}
async function updateRow(spreadsheetUrl, sheetName, rowNumber, values) {
    const auth = getAuth();
    const sheets = googleapis_1.google.sheets({ version: 'v4', auth });
    const spreadsheetId = extractSpreadsheetId(spreadsheetUrl);
    ...
    logger_1.logger.info(`Linha ${rowNumber} atualizada na aba ${sheetName}`);
}
async function getRows(spreadsheetUrl, sheetName) {
    const auth = getAuth();
    const sheets = googleapis_1.google.sheets({ version: 'v4', auth });
    const spreadsheetId = extractSpreadsheetId(spreadsheetUrl);
    try {
        ...
        logger_1.logger.error(`Erro ao ler aba ${sheetName}: ${error}`);
        // Retornar array vazio se a aba não existir ou o range for inválido
        return [];
    }
}
async function getNextId(spreadsheetUrl, sheetName) {
    const auth = getAuth();
    const sheets = googleapis_1.google.sheets({ version: 'v4', auth });
    const spreadsheetId = extractSpreadsheetId(spreadsheetUrl);
    const response = await sheets.spreadsheets.values.get({
...
async function validateConnection(spreadsheetUrl) {
    try {
        const auth = getAuth();
        const sheets = googleapis_1.google.sheets({ version: 'v4', auth });
        const spreadsheetId = extractSpreadsheetId(spreadsheetUrl);
        ...
        logger_1.logger.error('Erro ao validar conexão com planilha: ' + error);
        return false;
    }
}
async function listSheets(spreadsheetUrl) {
    const auth = getAuth();
    const sheets = googleapis_1.google.sheets({ version: 'v4', auth });
    const spreadsheetId = extractSpreadsheetId(spreadsheetUrl);
    const response = await sheets.spreadsheets.get({ spreadsheetId });
    const sheetNames = response.data.sheets?.map((sheet) => sheet.properties?.title).filter((title) => title !== undefined) || [];
    logger_1.logger.info(`Abas listadas: ${sheetNames.join(', ')}`);
    return sheetNames;
}
async function validateFarm(spreadsheetUrl, farmId, linkPosition = 1) {
    const auth = getAuth();
    const sheets = googleapis_1.google.sheets({ version: 'v4', auth });
    const spreadsheetId = extractSpreadsheetId(spreadsheetUrl);
        ...
        try {
            ...
            if (values && values.length > 0 && values[0].length > 0) {
                ...
                if (idInCell.toLowerCase() === farmId.toLowerCase()) {
                    ...
                    logger_1.logger.info(`Fazenda encontrada: ${sheetName}, ID: ${idInCell}, Nome: ${nomeInCell}, Link (posição ${linkPosition}): ${extractedUrl}`);
                    return { success: true, farmName: nomeInCell || sheetName, farmSheetUrl: extractedUrl || undefined };
                }
            }
        }
        catch (error) {
            logger_1.logger.error(`Erro ao buscar na aba ${sheetName}: ${error}`);
            // Continua para a próxima aba
        }
    }
    logger_1.logger.warn(`ID ${farmId} não encontrado em nenhuma aba`);
    return { success: false };
}
async function getSubtiposDaFazenda(spreadsheetUrl, farmId, tipo) {
    ...
    if (colunaIndex === undefined) {
        logger_1.logger.warn(`Tipo '${tipo}' não mapeado para coluna na planilha de cadastro`);
        return [];
    }
    try {
        ...
        if (!values || values.length === 0) {
            logger_1.logger.warn(`Nenhum dado encontrado na coluna ${colunaLetra} da planilha de cadastro`);
            return [];
        }
        ...
        if (subtipos.length > 0) {
            logger_1.logger.info(`Subtipos encontrados para ${tipo} na coluna ${colunaLetra}: ${subtipos.join(', ')}`);
            return subtipos;
        }
        else {
            logger_1.logger.warn(`Nenhum subtipo encontrado para ${tipo} na coluna ${colunaLetra}`);
            return [];
        }
    }
    catch (error) {
        logger_1.logger.error(`Erro ao buscar subtipos na planilha de cadastro: ${error}`);
        return [];
    }
}
/**
 ...
 */
async function findRowByInsumo(spreadsheetUrl, sheetName, insumoName) {
    const auth = getAuth();
    const sheets = googleapis_1.google.sheets({ version: 'v4', auth });
    const spreadsheetId = extractSpreadsheetId(spreadsheetUrl);
    try {
        ...
        logger_1.logger.warn(`Insumo "${insumoName}" não encontrado na aba ${sheetName}`);
        return null;
    }
    catch (error) {
        logger_1.logger.error(`Erro ao buscar insumo "${insumoName}" na aba ${sheetName}: ${error}`);
        return null;
    }
}
/**
 ...
 */
function calcularPrevisaoEstoque(qtdSaida, dataInicial, dataFinal, estoqueAtual) {
 ...
 */
async function calcularEstoque(spreadsheetUrl, insumoName) {
    const auth = getAuth();
    const sheets = googleapis_1.google.sheets({ version: 'v4', auth });
    const spreadsheetId = extractSpreadsheetId(spreadsheetUrl);
    let qtdEntrada = 0;
    ...
    try {
        ...
        logger_1.logger.error(`Erro ao calcular estoque para insumo "${insumoName}": ${error}`);
        return {
            qtdEntrada: 0,
            qtdSaida: 0,
            estoque: 0,
