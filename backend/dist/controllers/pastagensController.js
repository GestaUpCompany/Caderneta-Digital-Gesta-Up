...
exports.pastagensRouter.post('/calcular-tempo', async (req, res) => {
    const { planilhaUrl, pasto, tipo, dataAtual } = req.body;
    logger_1.logger.info(`Calcular tempo - pasto: ${pasto}, tipo: ${tipo}, dataAtual: ${dataAtual}`);
    ...
    try {
        ...
        const rows = await (0, googleSheetsService_1.getRows)(planilhaUrl, 'Troca de Pastos');
        logger_1.logger.info(`Registros encontrados: ${rows.length}`);
        ...
        logger_1.logger.info(`Registros filtrados para pasto ${pasto} (coluna ${colunaIndex}): ${registrosFiltrados.length}`);
        if (registrosFiltrados.length > 0) {
            logger_1.logger.info(`Primeiro registro filtrado: ${JSON.stringify(registrosFiltrados[0].row)}`);
        }
        if (registrosFiltrados.length === 0) {
        ...
        logger_1.logger.info(`Data atual parseada: ${dataAtualDate}`);
        if (!dataAtualDate) {
        ...
        const registrosOrdenados = registrosFiltrados
            ...
            logger_1.logger.info(`Data registro: ${row[1]}, parseada: ${dataRegistro}`);
        ...
        logger_1.logger.info(`Registros ordenados (data <= atual): ${registrosOrdenados.length}`);
        ...
        logger_1.logger.info(`Registro mais recente: ${JSON.stringify(registroMaisRecente.row)}`);
        logger_1.logger.info(`Data referência parseada: ${dataReferencia}`);
        if (!dataReferencia) {
            return res.json({
                success: true,
        ...
        logger_1.logger.info(`Tempo calculado para pasto ${pasto} (${tipo}): ${tempo}`);
        return res.json({
            success: true,
            tempo,
        ...
        logger_1.logger.error(`Erro ao calcular tempo para pasto ${pasto}: ${error}`);
