-- Fix: v_dest_form RECORD não era atribuído quando lote destino não tinha formulacao_id
-- Erro: 55000 "record v_dest_form is not assigned yet"
-- Causa: SELECT * INTO v_dest_form só executava dentro de IF v_dest_lote.formulacao_id IS NOT NULL
--        mas COALESCE(v_dest_form.nome, ...) era acessado incondicionalmente
-- Solução: substituir RECORD por variável escalar text, que é NULL por padrão
