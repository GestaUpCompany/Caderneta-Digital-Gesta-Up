...
- **Fazendas sem creep**: quando não há dieta vinculada (`temCreepDisponivel` falso), o payload fica byte-identico ao formato anterior — `nCabecasLote` total do lote (sem descontar ao pé), `qtdBezerrosLote` com o campo histórico do lote e `categorias`/`categoriasString` listando todas as categorias. Com dieta, a linha 'lote' grava `n_cabecas` só de adultos + `qtd_bezerros = 0` (o denominador `n_cabecas - qtd_bezerros` do cálculo de consumo resulta nos adultos; mandar o campo histórico descontaria o bezerro duas vezes — na Fazenda Sirio `lote_categorias.qtd_bezerros` vem replicado por linha e a soma infla o valor).
